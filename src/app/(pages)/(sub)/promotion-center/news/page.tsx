...
export default async function PromotionCenterNews({params, searchParams : {lang, page}} : any) {
    const language = await fetchLanguage(lang)
    const cookie = cookies()
    const cookieLang : any = cookie.get('LANG') || 'kr'
    const langValue = lang || cookieLang?.value
    const response = await api.get(`/user/promotion/getContentsList.php?contentType=${1}&businessDivisionType=0&userLang=${langValue}&page=${page || 1}&size=10&sortColumn=date&sortOrder=desc`)
