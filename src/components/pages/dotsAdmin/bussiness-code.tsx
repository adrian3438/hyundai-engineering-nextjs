...
export default function BussinessCodeViewPage ({id} : Props) {
    ...
    async function save () {
        try {
            ...
            if(id){
                try{
                    const response = await api.post(`/admin/code/updBusinessDivisionType.php` , formData)
                ...
                try{
                    const response = await api.post(`/admin/code/setBusinessDivisionType.php`, formData)
                    if(response?.data?.result === true) {
                        alert(response?.data?.resultMsg); 
                        router.push(`/dotsAdmin/common-code-management/bussiness-code-list`)
                    }
                    else{alert(response?.data?.resultMsg);}
                }catch{alert('Server Error')}
            }
    ...
    useEffect(()=> {
        async function fetchDetail() {
            if (id) {
                try {
                    const response = await api.get(`/admin/code/getBusinessDivisionTypeDetail.php?codeId=${id}`)
                    if (response?.data?.result === true) {
